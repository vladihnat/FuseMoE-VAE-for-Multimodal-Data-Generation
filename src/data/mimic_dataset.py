...
class MIMICTsTabDataset(Dataset):
    """Dataset for MIMIC-IV irregular TS + tabular samples in FuseMoE VAE format.

    Expects pickle files produced by data/preprocess_data.py.

    Each sample must contain:
        id:        int                  ICU stay identifier
        ts_values: (T, ts_dim)  float32 observed values at each timestamp
        ts_times:  (T,)         float32 timestamps in hours since ICU admission
        ts_mask:   (T, ts_dim)  float32 binary observation mask (1 = observed)
        tab_num:   (n_num,)     float32 numeric tabular features
        tab_cat:   (n_cat,)     int64   encoded categorical features
        label:     int                  in-hospital mortality label (0/1)
    """

    def __init__(self, pkl_path: PathLike) -> None:
    ...
    def _validate_sample(self, sample: Dict[str, Any]) -> None:
        required = {"id", "ts_values", "ts_times", "ts_mask", "tab_num", "tab_cat", "label"}
...
def mimic_ts_tab_collate_fn(batch: Sequence[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    """Pad variable-length irregular time series and stack tabular and label fields."""
    if len(batch) == 0:
...
def make_mimic_ts_tab_dataloader(
    pkl_path: PathLike,
    batch_size: int = 32,
    shuffle: bool = True,
    num_workers: int = 0,
    ...
    dataset = MIMICTsTabDataset(pkl_path=pkl_path)
    return DataLoader(
        dataset,
        ...
        collate_fn=mimic_ts_tab_collate_fn,
    )


__all__ = [
    "MIMICTsTabDataset",
    "mimic_ts_tab_collate_fn",
    "make_mimic_ts_tab_dataloader",
]
